...
function App() {
  ...
  function formatDate(dateString) {
    ...
    date.setUTCHours(0, 0, 0, 0); // set the time zone offset to UTC

    return date.toLocaleDateString('en-US', options);
  }

  function HourCard({ forecastDay }) {
    const timeString = forecastDay.time;
      ...
      <>
        <div className="hourContainer">
          ...
          <div>
            ...
            <p>{Math.round(forecastDay.temp_f)}°</p>
          </div>
        </div>
      </>
    );
  ...
  const fetchData = (e) => {
    ...
    axios
      .get(
        `http://api.weatherapi.com/v1/forecast.json?key=b64a8bb72f2047a0bf4191126230804&q=${location}&days=3&aqi=yes&alerts=no
        `
      )
  ...
  const handleSelection = (index) => {
    setSelection(index);
    hourlyWrapperRef.current.scrollTop = 0;
  };
          ...
          {data && (
            <>
              <div className="topDetails">
                ...
                <p className="temp">{Math.round(data.current.temp_f)}°</p>

                <div className="windHumidity">
                  <div className="whContainers">
                    <img className="smallIconWind" src={wind} />
                    <p>{Math.round(data.current.wind_mph)} mi/h</p>
                  </div>
                  <div className="whContainers">
                    <img className="smallIconWater" src={water} />
                    <p>{data.current.humidity}%</p>
                  </div>
                </div>
              </div>

              <div className="futureSelector">
                <div className="Dates">
                  {data.forecast.forecastday.map((day, inx) => (
                    <p key={inx} onClick={() => handleSelection(inx)}>
                      {formatDate(day.date)}
                    </p>
                  ))}
                </div>

                <div className="hourlyContainerWrapper" ref={hourlyWrapperRef}>
                  {data.forecast.forecastday[selection].hour
                    .slice(selection === 0 ? time : 0)
                    .map((forecastDay, index) => (
                      <HourCard forecastDay={forecastDay} key={index} />
                    ))}
                </div>
              </div>
            </>
